...
. Record "Datocratic Decisions" (DD) in a public GitHub repository using link:https://gitlab.eclipse.org/eclipse/asciidoc-lang/asciidoc-lang[AsciiDoc] documents
...
. Make the intent of each DD explicit by including both the "Problems to be Solved" (PS) and "We Should" (WS) recommendations (as practiced by link:https://www.forwardparty.com/american-scorecard[The Forward Party]).
